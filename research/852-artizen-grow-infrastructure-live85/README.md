...
| 5 | CONFIRM the "crew mandatory" rule against the replay before telling creators | The word "crew" appears zero times in the Grow bundle. What DOES ship: claims have a "people" section where projects add up to 5 collaborators (name + email + context all required to count). So team-ness is structurally rewarded, but a hard "no crew, no project" platform rule is NOT visible in the app - the livestream is the only source so far. Until confirmed, coach ZAO projects to fill all 5 collaborator slots regardless: it costs nothing and matches where the platform is pushing. |
...
|---|---|
| "Get a crew - mandatory for every project" | NOT found in app or any written source. Livestream-only so far. Confirm in replay/Telegram before repeating as a rule. |
